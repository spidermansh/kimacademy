...
export default function StudentManagement({
  ...
  const sessionsRemainingMap = useMemo<Record<string, number>>(() => {
    ...
    students.forEach(student => {
      const summary = computeTuitionSummary(student, transactions, attendance);
      map[student.id] = summary.sessionsRemaining;
    });
    return map;
  ...
  const loadStudents = async (): Promise<any[]> => {
    try {
      ...
      setEnrollments(enr);
      const populated = data.map((s: any) => populateStudentEnrollment(s, enr));
      setStudents(populated);
      onStudentsUpdated?.(populated);
  ...
  const handleExportExcel = () => {
    try {
      ...
      const dataToExport = filtered.map((s, idx) => ({
        ...
        'Lớp học': s.classNames ? s.classNames.join(', ') : (s.className || ''),
        'Giới tính': s.gender || '',
        'Năm sinh': s.birthYear || '',
        'SĐT phụ huynh': s.parentPhone || '',
        'Email phụ huynh': s.parentEmail || '',
        'Học phí/buổi (VNĐ)': s.activeEnrollments && s.activeEnrollments.length > 0
          ? s.activeEnrollments.map((e: any) => `${e.className}: ${Number(e.feePerSession).toLocaleString('vi-VN')}đ`).join('\n')
          : (s.feePerSession || 0),
        'Số buổi còn lại': s.activeEnrollments && s.activeEnrollments.length > 0
          ? s.activeEnrollments.map((e: any) => `${e.className}: ${e.sessionsRemaining ?? 0} buổi`).join('\n')
          : 0,
  ...
  const filtered = students.filter(s => {
    ...
    const matchSearch = !searchLower || 
      ...
      s.classNames?.some((c: string) => c.toLowerCase().includes(searchLower)) ||
      s.parentPhone?.includes(searchLower);

    const matchClass = !filterClass || s.classNames?.includes(filterClass);
  ...
  const studentClasses = [...new Set(
    students.flatMap(s => s.classNames || []).filter(Boolean)
  )].sort();
                  ...
                  <td className="px-4 py-3.5">
                    {student.activeEnrollments && student.activeEnrollments.length > 0 ? (
                      <div className="flex flex-wrap gap-1 max-w-[200px]">
                        {student.activeEnrollments.map((enr: any) => (
                          <span key={enr.id} className="inline-flex items-center gap-1 bg-indigo-50 text-indigo-700 text-[11px] font-semibold px-2 py-0.5 rounded-full border border-indigo-100">
                  ...
                  <td className="px-4 py-3.5 text-right font-medium">
                    {student.activeEnrollments && student.activeEnrollments.length > 0 ? (
                      <div className="flex flex-col items-end gap-1">
                        {student.activeEnrollments.map((enr: any) => {
                          const rem = enr.sessionsRemaining ?? 0;
                          return (
                            <div key={enr.id} className="flex items-center gap-1.5 whitespace-nowrap">
                              ...
                              <span className="text-[10px] text-slate-400">({enr.className})</span>
                            </div>
                          );
                        })}
                      </div>
                  ...
                  <td className="px-4 py-3.5 text-right font-medium">
                    {student.activeEnrollments && student.activeEnrollments.length > 0 ? (
                      <div className="flex flex-col items-end gap-0.5">
                        {student.activeEnrollments.map((enr: any) => (
                          <div key={enr.id} className="text-xs text-slate-500 whitespace-nowrap">
                            <span className="font-semibold text-emerald-700">{Number(enr.feePerSession).toLocaleString('vi-VN')}đ</span>
                            <span className="text-[10px] text-slate-400 font-normal font-sans"> ({enr.className})</span>
                          </div>
                        ))}
                      </div>
          ...
          >
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                {!editingStudent ? (
                  <div>
                    <label className="block text-xs font-semibold text-slate-500 mb-1.5 uppercase tracking-wider">Lớp học tuyển sinh</label>
                    {(classes || []).length > 0 ? (
                      <select
                      ...
                      >
                        ...
                        {(classes || []).filter(c => c.type !== 'online').map(cls => (
                      ...
                      <input
                        ...
                        className="w-full px-4 py-2.5 border border-slate-300 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none font-medium"
                      />
                    )}
                  </div>
                ) : (
                  <div>
                    <label className="block text-xs font-semibold text-slate-500 mb-1.5 uppercase tracking-wider">Lớp học đang học và Học phí</label>
                    <div className="bg-slate-50 rounded-xl p-3 border border-slate-200 space-y-2">
                      {editingStudent.activeEnrollments && editingStudent.activeEnrollments.length > 0 ? (
                        editingStudent.activeEnrollments.map((enr: any) => (
                          <div key={enr.id} className="flex flex-col gap-2 bg-white p-3 rounded-lg border border-slate-100 shadow-sm text-xs">
                            <div className="flex items-center justify-between">
                              ...
                              <button
                                ...
                                onClick={async () => {
                                  const ok = await toast.confirm({
                                    title: `Gỡ học sinh khỏi lớp ${enr.className}?`,
                                    message: `Lớp ${enr.className} sẽ kết thúc enrollment vào ngày hôm nay.`,
                                    confirmText: 'Gỡ lớp',
                                    danger: true
                                  });
                                  if (!ok) return;
                                  try {
                                    ...
                                    await api.deleteEnrollment(enr.id, todayIso);
                                    toast.success('Đã gỡ lớp thành công!', enr.className);
                                    const updatedStudents = await loadStudents();
                                    const updatedSelf = updatedStudents.find(s => s.id === editingStudent.id);
                                    if (updatedSelf) {
                                      setEditingStudent(updatedSelf);
                                    }
                                  } catch (err: any) {
                                    toast.error('Lỗi gỡ lớp', err.message);
                                  }
                                }}
                            ...
                            <div className="flex items-center justify-between pt-1.5 border-t border-slate-50 text-xs text-slate-500">
                              <div>
                                <span className="text-slate-400">Học phí: </span>
                                <span className="font-bold text-slate-700">{new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(enr.feePerSession)}</span>
                              </div>
                              <button
                                type="button"
                                onClick={() => {
                                  localStorage.setItem('tuition_search_query', editingStudent.name);
                                  onNavigate?.('hoc-phi');
                                  setShowForm(false);
                                }}
                                className="px-2 py-1 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-semibold rounded transition-colors cursor-pointer flex items-center gap-1"
                              >
                        ...
                        <div className="text-slate-400 text-xs italic text-center py-1">Chưa xếp lớp</div>
                      )}
                    </div>
                  </div>
                )}
                {editingStudent && (
                  <div className="col-span-2 bg-indigo-50/50 border border-indigo-100 rounded-2xl p-4 space-y-3 mt-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      <div>
                        <label className="block text-[10px] font-bold text-slate-500 mb-1 uppercase tracking-wider">Chọn lớp mới</label>
                        <select
                          id="new-enroll-class"
                          className="w-full px-3 py-2 bg-white border border-slate-300 rounded-xl text-xs focus:ring-2 focus:ring-indigo-500 outline-none font-medium cursor-pointer"
                        >
                          <option value="">-- Chọn lớp --</option>
                          {(classes || [])
                            .filter(c => c.type !== 'online' && !(editingStudent.classNames || []).includes(c.name))
                            .map(cls => (
                      ...
                      <div>
                        <label className="block text-[10px] font-bold text-slate-500 mb-1 uppercase tracking-wider">Ngày bắt đầu học</label>
                        <input
                          id="new-enroll-start"
                          type="date"
                    ...
                    <div className="flex justify-end pt-3">
                      <button
                        type="button"
                        onClick={async () => {
                          const classSelect = document.getElementById('new-enroll-class') as HTMLSelectElement;
                          const startInput = document.getElementById('new-enroll-start') as HTMLInputElement;
                          
                          const className = classSelect?.value;
                          const startDate = startInput?.value;

                          if (!className) {
                            toast.warning('Thiếu thông tin', 'Vui lòng chọn lớp học mới!');
                            return;
                          }
                          if (!startDate) {
                            toast.warning('Thiếu thông tin', 'Vui lòng chọn ngày bắt đầu học!');
                            return;
                          }

                          const currentCount = (editingStudent.activeEnrollments || []).length;
                          if (currentCount >= 3) {
                            const proceed = await toast.confirm({
                              title: 'Cảnh báo đăng ký lớp',
                              message: `Học sinh này đã đăng ký ${currentCount} lớp. Bạn có chắc muốn đăng ký thêm lớp thứ ${currentCount + 1}?`,
                          ...
                          try {
                            ...
                            const feePerSession = targetClass?.defaultFee || targetClass?.defaultFeePerSession || 0;

                            await api.addClassEnrollment({
                            ...
                            toast.success(`Đã thêm lớp ${className} cho học viên ${editingStudent.name}`);
                ...
                <div>
                  ...
                  <DateInput
                    ...
                    onChange={v => setForm(f => ({ ...f, enrollDate: v }))}
                    className="w-full px-4 py-2.5 border border-slate-300 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none flex items-center justify-between"
                  />
                </div>
