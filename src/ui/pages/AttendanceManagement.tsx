...
export default function AttendanceManagement({ students, classes, transactions }: { students: any[]; classes: any[]; transactions: any[] }) {
  ...
  const loadEnrollments = useCallback(async () => {
    try {
      ...
      console.error('Lỗi tải danh sách đăng ký lớp:', err);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    loadEnrollments();
  }, [loadAttendanceHistory, loadAllAttendance, checkTodayAttendance, loadEnrollments]);
  ...
  const studentsInClass = useMemo(() => {
    ...
    // Get all students matching this class name based on enrollment at selectedDate
    const matchingStudents = (students || []).filter(s =>
      enrollments.some(e =>
        e.studentId === s.id &&
        e.className.trim().toLowerCase() === selectedClassName.toLowerCase() &&
        selectedDate >= e.startDate &&
        (!e.endDate || selectedDate <= e.endDate)
      )
    );
    ...
    if (hasRecordsForDate) {
      ...
      return (students || []).filter(s =>
        (enrollments.some(e =>
          ...
          selectedDate >= e.startDate &&
          (!e.endDate || selectedDate <= e.endDate) &&
          (s.status !== 'suspended' && s.status !== 'left')
        )) || studentIdsRecorded.has(s.id)
      );
    } else {
      // Just return active students
      return matchingStudents.filter(s => s.status === undefined || s.status === 'active');
    }
  }, [selectedClassName, selectedDate, classAttendance, students, enrollments]);
              ...
              <div className="bg-slate-50 rounded-2xl p-4 border border-slate-200 flex items-center justify-between gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <DateInput
                    ...
                    className="px-4 py-2 border border-slate-300 rounded-xl text-sm font-semibold focus:ring-2 focus:ring-indigo-500 outline-none flex items-center justify-between"
                  />
                  <button
                    onClick={() => changeDate(1)}
                    className="p-2 bg-white border border-slate-300 rounded-lg hover:bg-slate-50 transition-colors"
                  ...
                  {auditTimeRangeType === 'week' && (
                    <div className="col-span-2">
                      ...
                      <div className="flex items-center gap-3">
                        <DateInput
                          value={auditWeekDate}
                          onChange={v => setAuditWeekDate(v)}
                          className="px-4 py-2 border border-slate-300 rounded-xl text-sm font-semibold text-slate-700 outline-none flex items-center justify-between"
                        />
                    ...
                    <div className="col-span-2 flex items-center gap-3">
                      <div className="flex-1">
                        ...
                        <DateInput
                          ...
                          onChange={v => setAuditCustomStart(v)}
                          className="w-full px-4 py-2 border border-slate-300 rounded-xl text-sm font-semibold text-slate-700 outline-none flex items-center justify-between"
                        />
                      </div>
                      <div className="flex-1">
                        ...
                        <DateInput
                          ...
                          onChange={v => setAuditCustomEnd(v)}
                          className="w-full px-4 py-2 border border-slate-300 rounded-xl text-sm font-semibold text-slate-700 outline-none flex items-center justify-between"
                        />
                      </div>
                    </div>
