...
export default function ParentReminder({ students, transactions, classes, attendance = [], enrollments = [] }: ParentReminderProps) {
  ...
  const reminderList = useMemo(() => {
    return students
      .filter(s => (s.classNames && s.classNames.length > 0 || s.className) && (s.status || 'active') === 'active')
      .map(student => {
        const summary = computeTuitionSummary(student, transactions, attendance, enrollments);
        const fee = summary.feePerSession;
        const totalPaid = summary.totalPaidOffline;
        const sessionsBought = summary.totalSessionsBought;
        const sessionsUsed = summary.totalSessionsUsed;
        const sessionsRemaining = summary.sessionsRemaining;
  ...
  }, [students, transactions, attendance, enrollments, contactRecords]);
  ...
  const filtered = useMemo(() => {
    ...
    if (searchText.trim()) {
      ...
      list = list.filter(s =>
        ...
        s.classNames?.some((c: string) => c.toLowerCase().includes(q)) ||
  ...
  const handleCopyTemplate = (text: string) => {
    ...
    if (templateStudent) {
      api.addAuditLog({
        ...
        entity: 'student',
        entityId: templateStudent.id,
        details: `Copy tin nhắn nhắc phụ huynh học viên ${templateStudent.name} (${templateStudent.urgency === 'expired' ? 'het-buoi' : 'sap-het'})`
  ...
  const handleMarkContacted = (studentId: string, method: 'zalo' | 'phone' | 'sms' | 'other') => {
    ...
    api.addAuditLog({
      ...
      entity: 'student',
      entityId: studentId,
      details: `Đã liên hệ nhắc nhở phụ huynh học viên ${student?.name || studentId} về học phí/điểm danh`
    }).catch(err => console.error(err));
  };
  ...
  const handleExportExcel = () => {
    ...
    const data = filtered.map((s, idx) => ({
      ...
      'Lớp': s.classNames ? s.classNames.join(', ') : (s.className || ''),
      ...
      'Lần liên hệ cuối': s.lastContact ? formatDate(s.lastContact.contactedAt) : 'Chưa liên hệ',
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden">
            ...
            <div className="p-5 overflow-y-auto flex-1 space-y-4">
              ...
              <div className="space-y-3">
                ...
                {(() => {
                  const displayClassNames = templateStudent.classNames ? templateStudent.classNames.join(', ') : (templateStudent.className || '');
                  return templateStudent.urgency === 'expired' ? (
                    <>
                      <TemplateCard
                        title="🚨 Thông báo hết buổi"
                        text={ZALO_TEMPLATES.fee_expired(templateStudent.name, templateStudent.vietAnhName || '', displayClassNames)}
                        onCopy={handleCopyTemplate}
                        urgent
                      />
                      <TemplateCard
                        title="😊 Nhắc nhẹ nhàng"
                        text={ZALO_TEMPLATES.gentle_remind(templateStudent.name, templateStudent.vietAnhName || '', templateStudent.sessionsRemaining, displayClassNames)}
                        onCopy={handleCopyTemplate}
                      />
                    </>
                  ) : (
                    <>
                      <TemplateCard
                        ...
                        text={ZALO_TEMPLATES.fee_warning(templateStudent.name, templateStudent.vietAnhName || '', templateStudent.sessionsRemaining, displayClassNames)}
                        onCopy={handleCopyTemplate}
                      />
                      <TemplateCard
                        title="😊 Nhắc nhẹ nhàng"
        ...
        <div className="flex-1 min-w-0">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-0.5 text-xs text-slate-500">
                <span className="font-medium text-indigo-600">
                  {student.classNames && student.classNames.length > 0 ? student.classNames.join(', ') : (student.className || '—')}
                </span>
          ...
          {student.lastContact && (
            <div className="mt-2 flex items-center gap-1.5 text-[10px] text-emerald-600 bg-emerald-50 px-2 py-1 rounded-md w-fit">
              <CheckCircle2 className="w-3 h-3" />
              <span>
                Đã liên hệ {formatDate(student.lastContact.contactedAt)}
