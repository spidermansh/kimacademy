...
export default function TeacherHome({ teacherName, classes, students, onNavigate }: TeacherHomeProps) {
  ...
  const myStudents = useMemo(() =>
    students.filter(s => myClasses.some(c => s.classNames?.includes(c.name) || s.className === c.name)),
  [students, myClasses]);
        ...
        {todayClasses.length === 0 ? (
          ...
          <div className="divide-y divide-slate-50">
            {todayClasses.map(cls => {
              const classStudents = students.filter(s => s.classNames?.includes(cls.name) || s.className === cls.name);
