...
import { StaffMember, MonthlySalary, AppSettings } from '../types';
...
export default function SalaryDashboard({ staff, classes, settings }: Props) {
  ...
  const [editValues, setEditValues] = useState<{ otherIncome: number; kpiDeduction: number; notes: string }>({ otherIncome: 0, kpiDeduction: 0, notes: '' });
  ...
  const handleSaveEdit = async (salary: MonthlySalary) => {
    try {
      const updated = {
        ...salary,
        otherIncome: editValues.otherIncome,
        kpiDeduction: editValues.kpiDeduction,
        notes: editValues.notes,
      };
      // Recalculate with carry-over
      const grossSalary = updated.teachingIncome + updated.baseSalary + updated.otherIncome - updated.kpiDeduction;
      updated.grossSalary = grossSalary;
      updated.taxAmount = Math.round(grossSalary * updated.taxRate);
      const payableAmount = grossSalary - updated.taxAmount;
      ...
      await api.updateMonthlySalary(salary.id, updated);
      setSalaries(prev => prev.map(s => s.id === salary.id ? { ...s, ...updated } : s));
      setEditingRow(null);
    } catch (err: any) {
      toast.error('Lỗi', err.message);
    }
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                ...
                <th className="text-right px-3 py-3 font-semibold text-slate-600">Lương khác</th>
                <th className="text-right px-3 py-3 font-semibold text-slate-600">Trừ KPI</th>
                <th className="text-right px-3 py-3 font-semibold text-slate-600 bg-blue-50">Thành tiền</th>
                <th className="text-right px-3 py-3 font-semibold text-red-600">Thuế 10%</th>
                <th className="text-right px-3 py-3 font-semibold text-amber-600">Ứng</th>
                <th className="text-right px-3 py-3 font-semibold text-green-600 bg-green-50">Thực nhận</th>
                  ...
                  return (
                    <tr key={s.id} className="border-b border-slate-100 hover:bg-lime-50/30 transition-colors">
                      ...
                      <td className="px-3 py-2.5 text-center">
                        <span className={`text-xs font-semibold px-1.5 py-0.5 rounded ${s.role === 'teacher' ? 'bg-blue-100 text-blue-700' : 'bg-violet-100 text-violet-700'}`}>
                          {s.role === 'teacher' ? 'GV' : 'VP'}
                        </span>
                      </td>
                      <td className="px-3 py-2.5 text-right">{s.role === 'teacher' ? `${s.totalSessions} × ${formatCurrency(s.ratePerSession)}` : '—'}</td>
                      <td className="px-3 py-2.5 text-right font-medium">{s.role === 'teacher' ? formatCurrency(s.teachingIncome) : '—'}</td>
                      <td className="px-3 py-2.5 text-right">{formatCurrency(s.baseSalary)}</td>
                      <td className="px-3 py-2.5 text-right">
                        {isEditing ? (
                          <input type="number" value={editValues.otherIncome} onChange={e => setEditValues({ ...editValues, otherIncome: +e.target.value })}
                            className="w-24 px-2 py-1 border rounded text-right text-xs" />
                        ) : formatCurrency(s.otherIncome)}
                      </td>
                        ...
                        <div className="flex items-center justify-center gap-1">
                          {isEditing ? (
                            ...
                            <>
                              {s.status === 'draft' && (
                                <>
                                  <button onClick={() => { setEditingRow(s.id); setEditValues({ otherIncome: s.otherIncome, kpiDeduction: s.kpiDeduction, notes: s.notes || '' }); }}
            ...
            {filtered.length > 0 && (
              <tfoot>
                <tr className="bg-slate-50 border-t-2 border-slate-300 font-bold text-sm">
                  <td colSpan={8} className="px-3 py-3 text-right text-slate-700">TỔNG CỘNG:</td>
              ...
              <div className="slip">
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '16px' }}>
                  ...
                  <div style={{ fontSize: '13px', textAlign: 'right' }}>
                    ...
                    <strong>{selectedSlip.role === 'teacher' ? 'Giáo viên' : 'Nhân viên VP'}</strong>
                  </div>
                </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', margin: '12px 0', fontSize: '13px' }}>
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td style={{ padding: '8px 12px', borderBottom: '1px solid #e2e8f0' }}>{selectedSlip.role === 'teacher' ? 'Lương học vụ' : 'Lương cơ bản'}</td>
                    ...
                    {selectedSlip.otherIncome > 0 && (
                      <tr>
                        <td style={{ padding: '8px 12px', borderBottom: '1px solid #e2e8f0' }}>Thu nhập khác</td>
                        <td style={{ padding: '8px 12px', borderBottom: '1px solid #e2e8f0', textAlign: 'right', fontWeight: 600 }}>{formatCurrency(selectedSlip.otherIncome)}</td>
                      </tr>
                    )}
                    ...
                    <tr>
                      <td style={{ padding: '8px 12px', borderBottom: '1px solid #e2e8f0', color: '#dc2626' }}>Thuế TNCN (10%)</td>
                      <td style={{ padding: '8px 12px', borderBottom: '1px solid #e2e8f0', textAlign: 'right', fontWeight: 600, color: '#dc2626' }}>-{formatCurrency(selectedSlip.taxAmount)}</td>
                    </tr>
